...
def consumer(pl, grafana_api, rrd_step, ttl, rk_tim , max_top , filter_mode):
    ...
    try:
        ...
        rrdtool.create("./rrd/Statistics.rrd", '--start', creationtimestring,
                       ...
                       'DS:N_Flow:GAUGE:{}:{}:{}'.format(hb, min, max),

                       'DS:MeanIn:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:MeanOut:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:MeanBoth:GAUGE:{}:{}:{}'.format(hb, min, max),

                       'DS:VarianceIn:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:VarianceOut:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:VarianceBoth:GAUGE:{}:{}:{}'.format(hb, min, max),

                       'DS:StdDevIn:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:StdDevOut:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:StdDevBoth:GAUGE:{}:{}:{}'.format(hb, min, max),

                       'DS:UpperBoundIn:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:LowerBoundIn:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:UpperBoundOut:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:LowerBoundOut:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:UpperBoundBoth:GAUGE:{}:{}:{}'.format(hb, min, max),
                       'DS:LowerBoundBoth:GAUGE:{}:{}:{}'.format(hb, min, max)
                       )
        ...
        while True:
            ...
            flow_streamer = NFStreamer(source=f_name, statistical_analysis=True,promiscuous_mode=True,accounting_mode=0)
            # Studying flows , if it's a new flow create the structure and rdd o/w update it
            for flow in flow_streamer:

                print(flow.application_name + " -> " + flow.src_ip + " -> " + flow.dst_ip + "[" + str(flow.src2dst_bytes) + "] "\
                ...
                if(filter_mode=="ip"):#Every flow is identified by external ip 
                    if ipaddress.ip_address(flow.dst_ip).is_private:# src is an external talker
                        if flow.src_ip not in talkers.keys():
                            ...
                            curr_talker = Talker(flow.src_ip, flow.application_name,flow.dst2src_bytes,flow.src2dst_bytes,"ip"
                                                ,flow.application_category_name, flow.application_is_guessed,
                                                flow.requested_server_name, flow.client_fingerprint, flow.server_fingerprint,
                                                flow.user_agent, flow.content_type)

                            # create his RRD
                            curr_talker.RRDcreate(rrd_step, hb, min, max, creationtimestring)
                            # insert in talkers
                        ...
                        if flow.dst_ip not in talkers.keys():
                            # create talker structure
                            curr_talker = Talker(flow.dst_ip, flow.application_name,flow.src2dst_bytes,flow.dst2src_bytes,"ip"
                                                ,flow.application_category_name, flow.application_is_guessed,
                                                flow.requested_server_name, flow.client_fingerprint, flow.server_fingerprint,
                                                flow.user_agent, flow.content_type)
                ...
                ###################################################################################
                    ...
                    if flow.application_name not in talkers.keys():
                        ...
                        if ipaddress.ip_address(flow.src_ip).is_private: #flow localhost -> talker
                            # create talker structure
                            curr_talker = Talker(flow.dst_ip, flow.application_name, flow.src2dst_bytes, flow.dst2src_bytes,"prot7"
                                                 , flow.application_category_name, flow.application_is_guessed,
                                                 flow.requested_server_name, flow.client_fingerprint, flow.server_fingerprint,
                                                 flow.user_agent, flow.content_type)
                            ...
                            # create talker structure
                            curr_talker = Talker(flow.dst_ip, flow.application_name, flow.dst2src_bytes, flow.src2dst_bytes,"prot7"
                                                 , flow.application_category_name, flow.application_is_guessed,
                                                 flow.requested_server_name, flow.client_fingerprint, flow.server_fingerprint,
                                                 flow.user_agent, flow.content_type)
            ...
            if len(talkers) > 0:
                ...
                if last_sort_in != t_sorted:
                    ...
                    if(filter_mode=="ip"): strings = [t.ip for t in t_sorted]
                    elif(filter_mode=="prot7"): strings = [t.prot7 for t in  t_sorted]
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        if answer == "y":
            ...
            print("Be sure to start grafana by yourlself")
            break
