...
class Talker:
    ...
    def __str__(self):
        if(self.filter=="ip"):
            return "\tTALKER IP: {}\n\tIN_BYTES: {}\n\tOUT_BYTES: {}\n\tIN+OUT_BYTES: {}\n".format(self.ip,self.prot7,self.in_bytes,self.out_bytes,self.inandout_bytes) \
                    +"\tCURRENT_IN_BYTES: {}\n\tCURRENT_OUT_BYTES: {}\n\tCURRENT_IN+OUT_BYTES: {}\n".format(self.in_bytes_current,self.out_bytes_current,self.inandout_bytes_current) \
                    +"\tAPPLICATION CATEGORY: {}\n\tGUESS DISSECTOR BASED OR PORT BASED: {}\n".format(self.application_category_name,self.guessType) \
                    +"\tREQUEST SERVER_NAME: {}\n\tCLIENT FINGERPRINT: {}\n\tSERVER FINGERPRINT: {}\n".format(self.requested_server_name,self.client_fingerprinting,self.server_fingerprinting) \
                    +"\tUSER AGENT: {}\n\tCONTENT TYPE:{}\n".format(self.user_agent,self.content_type)

        elif(self.filter=="prot7"):
            return "\tAPPLICATION PROTOCOL: {}\n\tIN_BYTES: {}\n\tOUT_BYTES: {}\n\tIN+OUT_BYTES: {}\n".format(self.prot7,self.in_bytes,self.out_bytes,self.inandout_bytes) \
                    +"\tCURRENT_IN_BYTES: {}\n\tCURRENT_OUT_BYTES: {}\n\tCURRENT_IN+OUT_BYTES: {}\n".format(self.in_bytes_current,self.out_bytes_current,self.inandout_bytes_current) \
                    +"\tAPPLICATION CATEGORY: {}\n\tGUESS DISSECTOR BASED OR PORT BASED: {}\n".format(self.application_category_name,self.guessType) \
                    +"\tREQUEST SERVER_NAME: {}\n\tCLIENT FINGERPRINT: {}\n\tSERVER FINGERPRINT: {}\n".format(self.requested_server_name,self.client_fingerprinting,self.server_fingerprinting) \
                    +"\tUSER AGENT: {}\n\tCONTENT TYPE:{}\n".format(self.user_agent,self.content_type)
