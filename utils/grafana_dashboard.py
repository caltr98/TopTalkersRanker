...
class DashBoard:
    ...
    def update_topin(self, in_sources):
        ...
        if (t_len > s_len):
            ...
            while(i < s_len):
                ...
                #Modify matcher
                self.json["panels"][4]['fieldConfig']['overrides'][i]['matcher']['options']=":RRD_"+ in_sources[i] +":In"
                #Modify properties
                self.json["panels"][4]['fieldConfig']['overrides'][i]['properties'][0]['value']='TOP'+ str(i+1) + 'In:' + in_sources[i]
                i+=1
        ...
        #new flow are move than saved , update and add the rest
            ...
            while(i < s_len):
                ...
                #Add source
                self.json["panels"][4]['targets'].append({"data":"", "refId": chr(i+65), "target": "RRD_" + in_sources[i] + ":In", "type": "timeseries"})
    ...
    def update_topout(self, out_sources):
        ...
        if (t_len > s_len):
            ...
            while(i < s_len):
                ...
                #Modify matcher
                self.json["panels"][6]['fieldConfig']['overrides'][i]['matcher']['options']=":RRD_"+ out_sources[i] +":Out"
                #Modify properties
                self.json["panels"][6]['fieldConfig']['overrides'][i]['properties'][0]['value']='TOP'+ str(i+1) + 'Out:' + out_sources[i]
                i+=1
        ...
        #new flow are move than saved , update and add the rest
            ...
            while(i < s_len):
                ...
                #Add source
                self.json["panels"][6]['targets'].append({"data":"", "refId": chr(i+65), "target": "RRD_" + out_sources[i] + ":Out", "type": "timeseries"})
                
                #Add tag and matcher
                override={"matcher":{"id":"byName","options":":RRD_" + out_sources[i] + ":Out"},
                          "properties":[{"id": "displayName","value": "TOP" + str(i+1) + "Out:"+out_sources[i]}]}
    ...
    def update_topboth(self, both_sources):
        ...
        if (t_len > s_len):
            ...
            while(i < s_len):
                ...
                #Modify matcher
                self.json["panels"][8]['fieldConfig']['overrides'][i]['matcher']['options']=":RRD_"+ both_sources[i] +":InOut"
                #Modify properties
                self.json["panels"][8]['fieldConfig']['overrides'][i]['properties'][0]['value']='TOP'+ str(i+1) + 'InOut:' + both_sources[i]
                i+=1
        ...
        #new flow are move than saved , update and add the rest
            ...
            while(i < s_len):
                ...
                #Add source
                self.json["panels"][8]['targets'].append({"data":"", "refId": chr(i+65), "target": "RRD_" + both_sources[i] + ":InOut", "type": "timeseries"})
                
                #Add tag and matcher
                override={"matcher":{"id":"byName","options":":RRD_" + both_sources[i] + ":InOut"},
                          "properties":[{"id": "displayName","value": "TOP" + str(i+1) + "InOut:"+both_sources[i]}]}
    ...
    def addbound(self, json, source, p_index):
        ...
        json["panels"][p_index]['targets'].append({"data": "", "refId": chr(i+65), "target": source, "type": "timeseries"})
    ...
    def addstdev(self, json, source, p_index):
        ...
        json["panels"][p_index]['targets'].append({"data" : "", "refId": "upper", "target": source, "type": "timeseries"})
        override = {"matcher": {"id": "byName", "options": source},
                    "properties": [{"id": "custom.lineStyle", "value": {"dash": [0, 10], "fill":"dot"}},
